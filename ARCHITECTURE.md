...
- **Materialization decision (`mustMaterialize`).** m[0] re-materializes ONLY on: first render, `project_memory_epoch` change (non-additive memory mutations: update/delete/archive/merge — additive write/promote deliberately do NOT bump, so new memories flow through m[1] instead), user-profile-version change, new compartment sequence, pending m[0] mutations, project-docs hash change, or upgrade-state change. **`maxMemoryId` is deliberately NOT a trigger** — it drives m[1] rendering, not m[0] invalidation; triggering on it would bust m[0] on every memory write and defeat the whole design. `session_facts` version is retired as a trigger (facts are memories now).
...
- **`m0_mutation_log` + `project_state`.** The mutation log carries m[0]-affecting deltas for m[1]; `project_state.project_memory_epoch` is the cross-process invalidation counter bumped by non-additive memory mutations and by `/ctx-session-upgrade` migration.
