...
| `language` | string | — | Output language for Magic Context's generated content and guidance. When set (e.g. "Turkish", "Español", "日本語", "Brazilian Portuguese"), the historian, dreamer, sidekick, and the agent-guidance block instruct the model to write its PROSE in this language while keeping all structural tokens (XML tags, the five memory category names, code identifiers, file paths) in English. Free-form: use the language's own name. USER-LEVEL ONLY (ignored in project config for security). Unset = today's behavior (model mirrors the conversation; English scaffolding). Changing it triggers one cache re-materialization; existing compartments/memories keep their original language until naturally rewritten. |
