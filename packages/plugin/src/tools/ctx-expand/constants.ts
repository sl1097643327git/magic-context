...
- ctx_expand(start=120, end=245, verbose=true) — lists each message SEPARATELY with its message id and a per-part preview (each tool call shown with its id and output size). Use this to find the exact message or tool call you want, then recover it in full by id.
- ctx_expand(id="msg_abc123") — returns the FULL untruncated content of that one message: every text part, and every tool call's complete input + output, read from stored history. This is the cheap way to get back a tool output you dropped with ctx_reduce — the original is still in storage even though the wire shows [dropped §N§]. If the message was deleted from history (session prune/revert), it says so.`;
